...
import logo from "assets/img/orchid_new_logo_uhbwk9 (1).png";
...
function Sidebar(props) {
  ...
  const createLinks = (routes) => {
    ...
    // Filter routes to exclude inactive ones
    const filteredRoutes = routes.filter(route => route.active !== false);
...
export function SidebarResponsive(props) {
  ...
  const { logoText, routes, secondary, ...rest } = props;
      ...
      <Drawer
        ...
        <DrawerContent
          ...
          <DrawerBody maxW='250px' px='1rem'>
            <Box maxW='100%' h='100vh' borderRadius='16px'>
              <Box pt={"25px"} mb='12px'>
                <Link
                  href={`${process.env.PUBLIC_URL}/#/`}
                  target='_blank'
...
Sidebar.propTypes = {
  // if true, then use the normal bg
